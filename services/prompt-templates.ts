...
const RAG_SYSTEM_PROMPT_TEMPLATE = `你是 ReseaKnowGPT 的「技术实践」助手。请严格基于下方知识库内容回答用户的技术问题。
...
1. 基于上方知识片段组织回答，代码示例直接引用片段中的原文并加注释说明。
2. 如果片段无法完整覆盖问题，可适当补充但须标注 > ⚠️ 以下为补充内容。
3. 回答正文中 **不要出现** 片段编号（如"片段 1"）。自然地组织回答即可。
4. 使用 Markdown 输出，代码块标注语言（如 \`\`\`jsx、\`\`\`bash）。
5. 回答末尾添加「📚 参考文档」，以 Markdown 链接形式列出来源页面的 URL。
6. 默认使用简体中文。`

export function buildRagSystemPrompt(ragContext: string, sourceUrls: string[]): string {
  const urlList = [...new Set(sourceUrls)]
    .map((url) => `- ${url}`)
    .join('\n')
  const urlHint = urlList
    ? `\n\n## 来源页面 URL（用于回答末尾的参考文档）\n\n${urlList}`
    : ''
  return RAG_SYSTEM_PROMPT_TEMPLATE.replace('{context}', ragContext) + urlHint
}
