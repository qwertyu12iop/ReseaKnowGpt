...
export default function WorkshopPage() {
  ...
  const toolLabels: Record<string, string> = {
    ...
    debug: t('workshop.debug'),
    language: t('workshop.language'),
    docs: t('workshop.docs'),
  }
                  ...
                  {tool.steps.map((step, i) => (
                    <div key={i} className="flex items-center shrink-0">
                      <div className="flex flex-col items-center">
                        <div
                          className={`w-5 h-5 rounded-full bg-gradient-to-br ${tool.gradient} flex items-center justify-center text-white text-[9px] font-bold opacity-70`}
                        >
                          {i + 1}
                        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
                {locale === 'zh'
                  ? '代码小工坊通过 Coze 工作流深度集成专属 AI Agent，覆盖代码生成、调试、审查、文档撰写四大环节，针对计算机专业特定语言（Python/C++/Java/TypeScript）和框架进行专项优化，并与文献库联动提供引用建议。'
                  : 'Code Workshop deeply integrates specialized AI agents via Coze workflows, covering code generation, debugging, review, and documentation — optimized for CS-specific languages (Python/C++/Java/TypeScript) with literature cross-reference suggestions.'}
              </p>
            </div>
          </div>
        </div>
