...
export async function POST(request: Request) {
  ...
  if (!apiKey || !workflowId) {
    return NextResponse.json(
      { error: '未配置 Coze 工作流凭证（COZE_API_KEY / COZE_LANG_SWITCH_WORKFLOW_ID）' },
      { status: 500 },
    )
  }
  ...
  try {
    ...
    if (!cozeRes.ok) {
      ...
      console.error(`[LanguageSwitch] Coze API Error ${cozeRes.status}:`, text)
    ...
    if (result.code !== 0) {
      console.error('[LanguageSwitch] Workflow Execution Failed:', result)
      return NextResponse.json(
